...
def test_first_request_is_allowed():
    ...
    limiter = LeakyBucket(capacity=3, leak_rate=1.0, storage=storage)
...
def test_capacity_is_enforced():
    storage = InMemoryStorage[LeakyBucketState]()
    limiter = LeakyBucket(capacity=3, leak_rate=0.0, storage=storage)
...
def test_users_are_rate_limited_independently():
    storage = InMemoryStorage[LeakyBucketState]()
    limiter = LeakyBucket(capacity=2, leak_rate=0.0, storage=storage)
